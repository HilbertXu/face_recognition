...
def build_model(self, dataset, nb_classes = 5):
    ...
    #构建一个空的网络模型，它是一个线性堆叠模型，各神经网络层会被顺序添加，专业名称为序贯模型或线性堆叠模型
        ...
        self.model.add(Convolution2D(32, 3, 3, border_mode='valid', 
        ...
        #keras模型具有两种模式即：训练模式和测试模式
        #而在测试模式中，一些正则化方法将不会被启用，例如Dropout层和BatchNormalization层
        #这大概就是在测试时模型精度很低的问题所在，当我在训练时将BN层去掉时，训练精度会一直卡在20附近
        
        self.model.add(BatchNormalization(epsilon=1e-06, mode=0, axis=-1, momentum=0.9, weights=None, beta_init='zero', gamma_init='one'))
        self.model.add(MaxPooling2D(pool_size=(2, 2)))                      #5 池化层
        self.model.add(Dropout(0.25))                                       #6 Dropout层

        self.model.add(Convolution2D(64, 3, 3, border_mode='valid'))         #7  2维卷积层
